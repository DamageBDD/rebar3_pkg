...
maybe_release_version(Profile, App) ->
    ...
    case file:consult(File) of
        {ok, [Terms]} ->
            case rebar3_pkg_version:find_release_vsn(Terms, App) of
...
resolve_version(Args, Profile, AppName, AppInfo, BaseDir) ->
    ReleaseFun = fun() -> maybe_release_version(Profile, AppName) end,
    {Vsn, Source} = rebar3_pkg_version:resolve(Args, BaseDir, AppInfo, ReleaseFun),
    rebar3_pkg_version:log_source("ipfs", Source),
    Vsn.
