...
init(State) ->
    Provider = providers:create([
        ...
        {desc, "Adds rebar.lock and each dependency directory to IPFS (recursive), "
...
current_profile(State) ->
    case rebar_state:current_profiles(State) of
        [P|_] -> atom_to_list(P);
        [] -> "default"
    end. %% mirrors your approach of reading the current profile. :contentReference[oaicite:1]{index=1}
...
maybe_release_version(Profile, App) ->
    ...
    case file:consult(File) of
        ...
        _ -> {error, no_releases}
    end. %% mirrors your RELEASES reader pattern. :contentReference[oaicite:2]{index=2}
...
find_release_vsn([H|T], App) ->
    case H of
        ...
        _ -> find_release_vsn(T, App)
    end;
find_release_vsn([], _) -> undefined. %% same shape as yours. :contentReference[oaicite:3]{index=3}
...
shell_escape(S) when is_list(S) ->
    ...
    case Needs of true -> [$' | esc_squotes(S)] ++ [$']; false -> S end;
...
pin_flag(Meta) -> case proplists:get_value(pin, Meta, false) of true -> true; _ -> false end.

ipfs_add_dir(Bin, Dir, Pin) ->
    %% -Q quiet CIDs only, -r recursive, -n hash only? We want real add:
    Cmd = string:join(
            [shell_escape(Bin), "add", "-Qr" | (case Pin of true -> ["--pin"]; _ -> [] end)]
            ++ [shell_escape(Dir)],
            " "),
...
ipfs_add_file(Bin, Path, Pin) ->
    Cmd = string:join(
            [shell_escape(Bin), "add", "-Q" | (case Pin of true -> ["--pin"]; _ -> [] end)]
            ++ [shell_escape(Path)],
            " "),
    Out = os:cmd("sh -c " ++ shell_escape(Cmd)),
    string:trim(Out).
...
add_dep(#{name := Name, vsn := Vsn0, dir := Dir}) ->
    Vsn = case Vsn0 of undefined -> ""; X -> X end,
